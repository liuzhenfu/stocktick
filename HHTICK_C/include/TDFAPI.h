...
//在调用期间，系统通知函数将收到MSG_SYS_CONNECT_RESULT，MSG_SYS_LOGIN_RESULT，MSG_SYS_CODETABLE_RESULT消息，如果前面的消息都是成功的话
//如果网络断开，则会收到MSG_SYS_DISCONNECT_NETWORK，
//pErr中存放错误代码，只有在错误代码为 TDF_ERR_NETWORK_ERROR 时候，外部才应该做重连逻辑
TDFAPI THANDLE TDF_Open(TDF_OPEN_SETTING* pSettings, TDF_ERR* pErr);
//TDF_OpenExt增加备选服务器，TDF会同时连接两台服务器获取数据，确保连接通畅
...
//TDF_OpenProxyExt增加备选服务器，TDF会同时连接两台服务器获取数据，确保连接通畅
TDFAPI THANDLE TDF_OpenProxyExt(TDF_OPEN_SETTING_EXT* pOpenSettings, TDF_PROXY_SETTING* pProxySettings, TDF_ERR* pErr);

//获取指定市场的代码表，如果为空，则获取所有的代码表，在已经收到MSG_SYS_CODETABLE_RESULT 消息之后，可以获得代码表
